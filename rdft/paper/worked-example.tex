...
For readers unfamiliar with AC, Appendix~\ref{app:ac_tutorial} provides
a self-contained tutorial with worked examples.
...
%% ================================================================
\section{Analytic Combinatorics in 10 Minutes}
\label{app:ac_tutorial}

This appendix is a self-contained tutorial on the AC tools used in the
main text. No prior knowledge of AC is assumed.

\subsection{What is a generating function and why should you care?}

Suppose you have a sequence of numbers: $a_0, a_1, a_2, \ldots$
(for instance, $a_n$ might count the number of trees with $n$ nodes).
A \textbf{generating function} packages the entire sequence into a
single function:
\[
A(z) = \sum_{n=0}^\infty a_n\, z^n = a_0 + a_1 z + a_2 z^2 + a_3 z^3 + \cdots
\]

\textbf{Why bother?} Because operations on the function correspond to
operations on the sequence. For example:
\begin{itemize}
...
A \textbf{singularity} of a function $A(z)$ is a point $z^*$ where the
function ceases to be ``well-behaved'' (in the mathematical sense:
ceases to be analytic, i.e.\ representable by a convergent power series).

\textbf{Everyday analogy.} Think of $A(z)$ as a road. A singularity is
a point where the road ends --- you cannot continue driving past it.
Different types of singularity correspond to different ways the road ends:
\begin{itemize}
\item \textbf{Pole} ($1/(1-z)$): the road goes to a cliff edge and drops
      off to infinity. The function literally blows up.
\item \textbf{Branch point} ($\sqrt{1-z}$): the road doesn't end, but it
      \emph{forks}. Past the singularity, there are two roads (the $+$ and
      $-$ square roots), and you must choose which one to follow. The
      function is finite at the fork but its derivative blows up.
\item \textbf{Logarithmic} ($\log(1-z)$): the road spirals slowly upward,
      never quite reaching infinity but never levelling off either.
\end{itemize}

\textbf{Why do singularities matter for counting?} Because the power
series $A(z) = \sum a_n z^n$ converges inside a disk of radius
$R = |z^*|$ (the distance to the nearest singularity) and diverges
outside. The \emph{rate} at which the coefficients grow is controlled by
\emph{how} the function fails at the singularity --- gentle failure
(branch point) means slow growth; violent failure (pole) means fast growth.

\subsection{Why singularities determine asymptotics: the complex analysis viewpoint}

The connection between singularities and coefficient growth is made
rigorous by \textbf{complex analysis} --- the theory of functions of
complex variables $z = x + iy$.

\textbf{Why complex numbers?} Even if you only care about real
coefficients $a_n$, the generating function $A(z)$ ``lives'' in the
complex plane. A singularity at $z^* = -1/4$ (a negative real number)
affects the growth of $a_n$ just as much as one at $z^* = 1/4$. You
cannot see this by looking only at real $z$; you need the full complex
picture.

The key formula is the \textbf{Cauchy coefficient formula}:
\[
a_n = [z^n]\,A(z) = \frac{1}{2\pi i} \oint_{|z|=r} A(z)\, z^{-n-1}\, \dd z
\qquad (r < R)
\]
This says: to extract $a_n$, integrate $A(z) \cdot z^{-n-1}$ around a
circle of radius $r$ in the complex plane. (The $2\pi i$ and the contour
are standard complex analysis; if you haven't seen this before, just
accept it as a machine that extracts coefficients.)

\textbf{The punchline.} As $n \to \infty$, the factor $z^{-n-1}$ becomes
exponentially large near $|z| = R$ (the singularity) and exponentially
small elsewhere. So the integral is dominated by a tiny neighbourhood of
the singularity. The \emph{shape} of the singularity (pole, branch point,
log) determines the \emph{value} of the integral, hence the growth of $a_n$.

This is why we care about classifying singularities: the classification
directly gives us the asymptotics.
