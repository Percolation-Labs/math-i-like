...
\begin{remark}[The honest summary]
For the Gribov one-loop $\beta$-function:
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{lll}
\toprule
\textbf{Quantity} & \textbf{AC computes?} & \textbf{Value} \\
\midrule
Diagram count $c_2$ & Yes (Lagrange) & $\beta - \chi$ \\
Symmetry factor & Yes (embedded in $c_2$) & $1/2$ \\
Singularity type & Yes (Newton polygon) & Square-root \\
$d_c$ & Yes (vertex dimension) & 4 \\
Transfer exponent & Yes (from type) & $n^{-3/2}$ \\
$\alpha_p$ & Yes (AC chain) & $(pd-2)/2$ \\
\midrule
Loop integral pole & No (requires $D$, $d$) & $1/(4\pi^2 D^2\varepsilon)$ \\
$\beta$-function coeff $b_1$ & No (= $c_2 \times$ integral) & $1/(4\pi^2 D^2)$ \\
$\lambda^*$ (fixed point) & No (= $\varepsilon/b_1$) & $4\pi^2 D^2\varepsilon$ \\
Anomalous dimension $\eta$ & Open programme & $\varepsilon/12 + O(\varepsilon^2)$ \\
\bottomrule
\end{tabular}
\end{center}
The first six rows are what this paper computes. The last four are
what the RG adds. The division is clean: AC gives the
\emph{combinatorial skeleton}; the loop integral gives the
\emph{metric} (the $D$- and $d$-dependent weight per loop).
For universal quantities (exponents, universality class), the metric
cancels. For non-universal quantities (amplitudes, fixed-point location),
it doesn't.
\end{remark}
...
An important clarification: the AC route as developed in this paper
extracts only the \emph{leading} exponents (from the singularity type).
The standard RG additionally computes anomalous dimensions ($\eta$,
$\gamma$), corrections to scaling, and $\beta$-function coefficients.

However, there is no reason in principle why these quantities should
be inaccessible to AC. They are all encoded in the generating function:
\begin{itemize}
\item \textbf{Anomalous dimensions} correspond to the \emph{amplitude}
      $C$ in $G \sim G^* - C(G_0^* - G_0)^{1/2}$. We extract the
      exponent $1/2$ but discard $C$. The amplitude is determined by
      $\phi(G^*)$, $\phi''(G^*)$, and $G_0^*$ --- all algebraic
      quantities computable from the DSE kernel.
\item \textbf{Corrections to scaling} come from the subleading terms
      in the Puiseux expansion: $c_n \sim n^{-3/2}(1 + c_1/n + c_2/n^2
      + \cdots)$. These are computed by the \emph{full} transfer theorem
      (Flajolet--Sedgewick Theorem~VI.3 includes arbitrarily many
      correction terms), not just the leading order.
\item \textbf{$\beta$-function coefficients} encode how the effective
      coupling runs with scale. In the AC framework, the scale
      dependence enters through the $d$-dependent weight $\Omega_d$
      (Section~\ref{sec:combined}). The $\epsilon$-expansion is the
      Taylor expansion of the branch point location $z^*(d)$ around
      $d = d_c$.
\end{itemize}
In short: the RG information is \emph{already in} the generating
function. What we have developed here extracts only the coarsest
feature (the singularity type). Extracting the finer features
(amplitudes, corrections, running) from the AC side is an open
programme --- but one where the mathematical tools (Puiseux expansion,
$d$-dependent Lagrange equations) already exist.

What the AC route does provide, which the RG does not make manifest,
...
be opaque from the RG perspective.
