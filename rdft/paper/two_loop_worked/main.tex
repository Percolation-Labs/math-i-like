...
\item \textbf{The causal structure of non-equilibrium theories.}
In non-equilibrium systems (reaction-diffusion, directed
percolation), propagators are retarded: $G_0 = 1/(-i\omega + Dk^2)$
with causality enforced by the $i\omega$ prescription.  The
frequency contour integral produces combinatorial $\delta$-function
constraints (the ``causal fingerprint'' of each diagram) that
determine which Wick contractions contribute.

This causal structure is a physics input from the action, not a
graph-theoretic property.  AC handles the spatial/parametric part
of the integral; the causal part must be specified separately.
In practice, for DP, the rapidity-reversal symmetry
$\phit(x,t) \leftrightarrow -\phi(x,-t)$ constrains the causal
structure and halves the number of independent diagrams.
...
The single thing AC does not bypass is the transcendental period
of a non-factorisable graph --- but neither does any other method.
