...
exponents is the central problem of non-equilibrium statistical mechanics.

The standard approach, developed by Doi~\cite{doi1976} and Peliti~\cite{peliti1985}
and reviewed by T\"auber et al.~\cite{tauber2005}, maps the master equation to a
field theory, generates Feynman diagrams, evaluates loop integrals, performs
BPHZ renormalization via the Connes--Kreimer Hopf algebra~\cite{conneskreimer1998},
...
\item \textbf{BRW validation.} For the Gribov process (BRW), the auto-DSE
...
\begin{example}[Gribov Process / BRW]
\label{ex:gribov}
The branching random walk has three reactions:
\begin{align}
...
to a Feynman vertex (corolla) with $m$ outgoing $\tilde\phi$ legs and
...
The amplitude of a diagram $G$ in the parametric representation is:
\begin{equation}
...
\label{eq:parametric}
\end{equation}
where $\sigma_d = |E_{\mathrm{int}}| - (d/2)L$ is the superficial degree
of divergence and the $\delta(\Sigma_\ell)$ factors arise from
$\omega$-integration.
...
For pair annihilation $2A \to \emptyset$, the Ward identity from probability
conservation $\mathcal{H}(1, a) = 0$ cancels the quartic vertex exactly,
making $\alpha = d/2$ exact to all loop orders~\cite{lee1994}.

%% ================================================================
\section{Analytic Combinatorics: The Direct Route}
\label{sec:ac}

\subsection{The DSE as a Lagrange Equation}
...
The Puiseux exponent $p/q$ is determined by the Newton polygon of the shifted
polynomial $F(G^* + \delta G, G_0^* + \delta G_0)$. For a quadratic $\phi$
...
and $V(t) \sim t^{d_s/2}$. The substitution $d \to d_s$ in
...
$A{+}B \to \emptyset$ & two-species & 4 & $\sqrt{}$-branch & $\rho \sim t^{-d/4}$ & Lee--Cardy (1995) \\
BRW (Gribov) & $1+(\beta{-}\chi)G - \chi G^2$ & 4 & $\sqrt{}$-branch & $\langle a^p\rangle \sim t^{(pd-2)/2}$ & Bordeu+ (2019) \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}[leftmargin=2em]
\item \textbf{Sierpinski carpet} ($d_s \approx 1.86$): $\alpha_2 = (2 \times 1.86 - 2)/2 = 0.86$.
Simulation: $0.81(5)$. \checkmark

\item \textbf{Random tree} ($d_s = 4/3$): $\alpha_2 = (2 \times 4/3 - 2)/2 = 1/3$.
Simulation: $0.35(7)$. \checkmark

\item \textbf{Preferential attachment} ($d_s \geq 4$): mean-field,
...
Hopf antipode & Renormalization counterterms \\
...
\item Extracts interaction vertices (corollas)
...
\subsection{Step 3: Interaction Vertices (Corollas)}
...
\subsection{Step 1: Extract Corollas from the Liouvillian}

Each monomial $\tilde\phi^m\phi^n$ in the Liouvillian $\mathcal{Q}$ with
$m+n \geq 3$ defines a \textbf{corolla} --- a single vertex with $m$
outgoing half-edges ($\tilde\phi$ type) and $n$ incoming half-edges
($\phi$ type). For the Gribov process:

\begin{figure}[H]
\centering
% Corollas (amputated vertices) — cf. thesis Figure 3.3
\begin{tikzpicture}[>=Stealth, scale=0.8]
  % Quartic vertex (2,2): coupling -chi
  \fill (0.0,0.0) circle (2.5pt);
  \draw[->, thick] (0.0,0.0) -- (-0.60,0.00);
  \draw[->, thick] (0.0,0.0) -- (-0.30,0.52);
  \draw[<-, thick] (0.0,0.0) -- (0.30,0.52);
  \draw[<-, thick] (0.0,0.0) -- (0.60,0.00);
  \node[below=8pt, font=\scriptsize] at (0.0,-0.2) {$\tilde{\phi}^2\phi^2$: $-\chi$};
  % Cubic vertex (2,1): coupling beta
  \fill (2.5,0.0) circle (2.5pt);
  \draw[->, thick] (2.5,0.0) -- (1.90,0.00);
  \draw[->, thick] (2.5,0.0) -- (2.50,0.60);
  \draw[<-, thick] (2.5,0.0) -- (3.10,0.00);
  \node[below=8pt, font=\scriptsize] at (2.5,-0.2) {$\tilde{\phi}^2\phi^1$: $\beta$};
  % Cubic vertex (1,2): coupling -chi
  \fill (5.0,0.0) circle (2.5pt);
  \draw[->, thick] (5.0,0.0) -- (4.40,0.00);
  \draw[<-, thick] (5.0,0.0) -- (5.00,0.60);
  \draw[<-, thick] (5.0,0.0) -- (5.60,0.00);
  \node[below=8pt, font=\scriptsize] at (5.0,-0.2) {$\tilde{\phi}^1\phi^2$: $-\chi$};
  % Mass vertex (1,1): coupling beta-epsilon
  \fill (7.5,0.0) circle (2.5pt);
  \draw[->, thick] (7.5,0.0) -- (6.90,0.00);
  \draw[<-, thick] (7.5,0.0) -- (8.10,0.00);
  \node[below=8pt, font=\scriptsize] at (7.5,-0.2) {$\tilde{\phi}^1\phi^1$: $\beta{-}\epsilon$};
\end{tikzpicture}
\caption{Corollas of the Gribov process, generated automatically from the
stoichiometry matrix by the code \texttt{Liouvillian(network).vertices}.
Compare thesis Figure~3.3.
Outgoing arrows: $\tilde\phi$ (creation); incoming arrows: $\phi$ (annihilation).
The mass vertex $(1,1)$ contributes to the propagator, not to loop diagrams.}
\label{fig:corollas}
\end{figure}
...
half-edges to form internal propagators. Given a multiset of corollas,
the algorithm:

\begin{enumerate}[leftmargin=2em]
\item Chooses $n$ interaction vertices ($m+n \geq 3$) from the corolla set.
...
At one loop ($L = 1$), the canonical self-energy diagram is two vertices
