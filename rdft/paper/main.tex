...
\noindent Three representative diagrams with species-coloured edges:

\begin{figure}[H]
\centering
% Diagram 1: A-only self-energy (thesis Eq. 3.23a)
\begin{tikzpicture}[>=Stealth, scale=0.85]
  \fill (-1.2,0) circle (3pt); \fill (1.2,0) circle (3pt);
  \draw[->, thick, black, bend left=40] (-1.2,0) to
    node[above, font=\scriptsize] {$A$} (1.2,0);
  \draw[->, thick, black, bend left=40] (1.2,0) to
    node[below, font=\scriptsize] {$A$} (-1.2,0);
  \draw[<-, thin, dashed, gray] (-1.2,0) -- (-2.0,0.3);
  \draw[<-, thin, dashed, gray] (-1.2,0) -- (-2.0,-0.3);
  \node[below=12pt] at (0,0) {\scriptsize $\mathfrak{s}=2$, $c=\chi^2$};
  \node[above=8pt] at (0,0.5) {\scriptsize (a) A-only};
\end{tikzpicture}
\hspace{1.5cm}
% Diagram 2: Mixed A-B loop (transmutation)
\begin{tikzpicture}[>=Stealth, scale=0.85]
  \fill (-1.2,0) circle (3pt); \fill (1.2,0) circle (3pt);
  \draw[->, thick, black, bend left=40] (-1.2,0) to
    node[above, font=\scriptsize] {$A$} (1.2,0);
  \draw[->, thick, orange, dashed, bend left=40] (1.2,0) to
    node[below, font=\scriptsize, orange] {$B$} (-1.2,0);
  \draw[<-, thin, dashed, gray] (-1.2,0) -- (-2.0,0.3);
  \draw[->, thin, dashed, orange] (1.2,0) -- (2.0,0.3);
  \node[below=12pt] at (0,0) {\scriptsize $\mathfrak{s}=1$, mixed};
  \node[above=8pt] at (0,0.5) {\scriptsize (b) A{+}B loop};
\end{tikzpicture}
\hspace{1.5cm}
% Diagram 3: B-only loop
\begin{tikzpicture}[>=Stealth, scale=0.85]
  \fill (-1.2,0) circle (3pt); \fill (1.2,0) circle (3pt);
  \draw[->, thick, orange, dashed, bend left=40] (-1.2,0) to
    node[above, font=\scriptsize, orange] {$B$} (1.2,0);
  \draw[->, thick, orange, dashed, bend left=40] (1.2,0) to
    node[below, font=\scriptsize, orange] {$B$} (-1.2,0);
  \draw[<-, thin, dashed, gray] (-1.2,0) -- (-2.0,0.3);
  \draw[<-, thin, dashed, gray] (-1.2,0) -- (-2.0,-0.3);
  \node[below=12pt] at (0,0) {\scriptsize $\mathfrak{s}=2$, B-only};
  \node[above=8pt] at (0,0.5) {\scriptsize (c) B{-}B loop};
\end{tikzpicture}
\caption{Representative one-loop diagrams with species-coloured edges.
\textbf{(a)}~A-only: both internal propagators are A-species (black);
$\mathfrak{s}=2$ from vertex swap. This is the diagram that contributes
to the RG flow (thesis Eq.~3.24).
\textbf{(b)}~Mixed A+B: one A-edge and one B-edge (orange dashed);
$\mathfrak{s}=1$ because swapping vertices would exchange species.
\textbf{(c)}~B-only loop: both edges are B-species; $\mathfrak{s}=2$
from vertex swap. B-loops do not contribute to the RG because the immobile
B-propagator carries no loop momentum.}
