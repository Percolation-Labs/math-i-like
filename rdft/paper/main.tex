...
full two-species prion system on a 1D periodic lattice ($L = 500$,
$20{,}000$ realizations per parameter set), using the multi-species
extension of the Rust simulator \texttt{rdft-sim}.
Table~\ref{tab:prion_sim} shows the survival fraction across a scan
of~$R_0$, and Table~\ref{tab:prion_exponents} compares the measured
conditional volume exponents at near-criticality with the DP prediction.

\begin{table}[H]
\centering
\caption{Prion propagation on a 1D lattice ($L{=}500$, $20{,}000$
realisations): scan of $R_0$ from subcritical to supercritical.
The critical $R_0^c \approx 1.4$--$1.5$ is shifted above the
mean-field value $R_0^{\rm MF} = 1$ by spatial fluctuations.}
\label{tab:prion_sim}
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{rrcl}
\toprule
...
\midrule
0.8 & 0.040 & 0/20000   & subcritical \\
1.2 & 0.060 & 0/20000   & subcritical \\
1.4 & 0.070 & 2/20000   & \textbf{near-critical} \\
1.5 & 0.075 & 403/20000 & near-critical (above) \\
1.6 & 0.080 & 1192/20000 & supercritical \\
2.0 & 0.100 & 3214/20000 & supercritical \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Conditional volume exponents $\alpha_p^{\rm cond}$ for prion
propagation at $R_0 = 1.5$ ($d{=}1$, $L{=}500$, $20{,}000$ realisations).
The DP prediction is $\alpha_p^{\rm cond} = p\,d_s/2 = p/2$.
The systematic ${\sim}15\%$ excess is consistent with being slightly
above the critical point.}
\label{tab:prion_exponents}
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{cccc}
\toprule
$p$ & $\alpha_p^{\rm cond}$ (measured) & DP theory ($p/2$) & $\alpha_p/p$ \\
\midrule
1 & 0.577 & 0.500 & 0.577 \\
2 & 1.152 & 1.000 & 0.576 \\
3 & 1.724 & 1.500 & 0.575 \\
...
$R_0^c \approx 1.4$--$1.5$ (zero survival below, persistent survival above);
(ii)~the conditional volume exponents $\alpha_p^{\rm cond} / p \approx 0.576$,
close to the DP prediction $d_s / 2 = 0.5$ and nearly constant across
$p = 1, 2, 3$, confirming the single-parameter BRW scaling;
(iii)~the fluctuation shift $R_0^c / R_0^{\rm MF} \approx 1.4$, consistent
with DP in one dimension.
