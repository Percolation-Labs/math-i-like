...
\end{verbatim}
returns the Liouvillian, interaction vertices, DSE kernel $\phi(G)$,
...
\begin{itemize}[leftmargin=2em]
\item Ward identity warnings when all $n_{\rm in}$ are equal, with the
corrected physical $d_c$;
\item parity degeneracy warnings when the leading coefficient of $\phi(G)$
can vanish, identifying the critical manifold where the branch point
collapses to a pole;
\item higher-order branch detection when $\deg\phi \geq 3$, flagging
parameter values where the Puiseux exponent changes from $1/2$ to $1/3$.
\end{itemize}
