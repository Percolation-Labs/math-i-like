...
generically produces square-root branch points and $n^{-3/2}$ coefficient
...
Applying the pipeline to 14~CRNs from the reaction-diffusion literature
(Table~\ref{tab:crn_survey}) yields three structural insights.
First, all directed percolation systems share a quadratic DSE kernel with a
square-root branch, confirming the universality of the $n^{-3/2}$ transfer
exponent.
Second, the parity-conserving (BARW-even) system shows a qualitative
singularity change: the leading coefficient of $\phi(G)$ vanishes at
criticality, collapsing the branch point to a pole. This provides an
algebraic explanation for why the $\varepsilon$-expansion
fails~\cite{cardytauber1996}: perturbation theory around a branch point
cannot reach a pole.
Third, the PCPD system has a cubic DSE kernel admitting a cube-root branch
at a special parameter ratio, potentially resolving the long-standing debate
over its universality class~\cite{hinrichsen2001}.

The open-source implementation \texttt{rdft} makes this pipeline accessible
to any researcher who can specify a chemical reaction network. The automatic
singularity diagnostics detect Ward identities, parity degeneracies, and
higher-order branch points without manual intervention.
