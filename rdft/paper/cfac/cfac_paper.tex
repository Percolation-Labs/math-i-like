...
\begin{abstract}
A reaction network of physical interest---bacteria secreting a
chemoattractant, protein monomers in a fluctuating cytoplasm, ice
nucleation sites in supersaturated vapour---almost always sits in
a continuous fluctuating medium that couples back to its
kinetics.  Doi--Peliti (DP) handles the discrete reacting
species; Martin--Siggia--Rose (MSR) handles the continuous
medium; coarse-graining away either sector produces
\emph{spurious} entropy production.  This paper treats both on
equal footing.  The DP--MSR dressed-propagator generating
function of such a coupled \emph{CRN-in-medium} is already a
Lagrange equation in the sense of Flajolet--Sedgewick analytic
combinatorics---stated as a theorem in the companion note---which
imports four consequences wholesale: (C1) all-orders coefficient
asymptotics via the transfer theorem, (C2) classification of
mean-field universality by the Puiseux exponent $p/q$, (C3)
non-perturbative nucleation rates as a branch-gap contour
integral, and (C4) UV divergence structure by power counting on a
finite 1PI enumeration.  We call the resulting framework Coupled
Field Analytic Combinatorics (CFAC).  This paper elaborates (C1)--(C4) at
...
\textbf{Result 3}~a two-colour Kirchhoff factorisation
\emph{theorem} for DP--MSR KS self-energies at all loop orders
$L \geq 2$: the structural factor $p^{L-1}$ follows from the
perfect-matching structure forced on the field-coloured subgraph
by the $(\deg_P,\deg_F)=(2,1)$ vertex signature.  Two supporting results
...
branch-gap formula), and Theorem~\ref{thm:ksfactor} (two-colour
Kirchhoff factorisation for DP--MSR KS, with the structural
$p^{L-1}$ factor)---are all consequences of
...
via change of variables; Theorem~\ref{thm:ksfactor} is the
structural claim about how (4) behaves under bi-colouring of
edges, proved for DP--MSR KS via the perfect-matching structure
of the field-coloured subgraph.
...
\item Mass ratios $(m_\psi/m_\phi)$ and diffusion ratios
  ...
  evaluated at the saddle; the bi-coloured factorisation
  $\mathcal{K}(G;p,f) = p^{L-1}\,Q_G(p,f)$
  (Theorem~\ref{thm:ksfactor} for DP--MSR KS) splits these
  dependencies into single-colour factors.
\end{enumerate}
\end{proposition}
...
\subsection{Prefactor \texorpdfstring{$C$}{C}: Schl\"ogl-I worked example}
\label{sec:prefactor-schloegl}

The preceding establishes $S$ (the exponent) algebraically from
the branch-gap integral.  The prefactor $C$ in
$\tau = C\,\ee^{VS}$ is ``in principle'' determined by the
transfer theorem~\eqref{eq:transfer}; we now close that loop by
computing $C$ in closed form for the simplest bistable CRN and
verifying against Gillespie.  The calculation is a template for
every polynomial one-species network; the coupled-field case
studies of \S\ref{sec:applications} reduce to it under the
adiabatic-field approximation that gives the effective $w_\pm$.

\paragraph{Schl\"ogl-I.}
$2A \xrightarrow{k_1} 3A$, $3A \xrightarrow{k_2} 2A$,
$A \xrightarrow{k_3} \varnothing$,
$\varnothing \xrightarrow{k_4} A$, with intensive rates
$w_+(x) = k_4 + k_1 x^2$ and $w_-(x) = k_3 x + k_2 x^3$.  The
steady-state DSE $w_-(G) = z\,w_+(G)$ takes the Lagrange form
\begin{equation}\label{eq:schloegl-lagrange}
  G(z) = z\,\varphi(G),
  \qquad
  \varphi(G) = \frac{k_4 + k_1 G^2}{k_3 + k_2 G^2}.
\end{equation}
Eliminating $z$ from $F = z(k_4 + k_1 G^2) - (k_3 G + k_2 G^3) = 0$
and $\partial_G F = 0$ gives the branch-point biquadratic
\begin{equation}\label{eq:schloegl-branchpoly}
  k_1 k_2\,G_*^{4}
  + (3 k_2 k_4 - k_1 k_3)\,G_*^{2}
  + k_3 k_4 = 0,
\end{equation}
solvable in closed form; $z_* = (k_3 + 3 k_2 G_*^{2})/(2 k_1 G_*)$
and Puiseux exponent $p/q = 1/2$.

\paragraph{Puiseux amplitude (transfer-theorem prefactor).}
Expanding $F$ at $(G_*, z_*)$, the square-root Puiseux amplitude
is
\begin{equation}\label{eq:schloegl-K}
  \mathcal K
  = \sqrt{\dfrac{k_4 + k_1 G_*^{2}}{k_1 z_* - 3 k_2 G_*}},
\end{equation}
an explicit algebraic function of the rate constants.  By
Flajolet--Sedgewick singularity analysis,
$[z^n] G(z) \sim (\mathcal K/2\sqrt{\pi})\,z_*^{-n}\,n^{-3/2}$.

\paragraph{From $\mathcal K$ to the MFPT prefactor.}
The transfer-theorem amplitude $\mathcal K$ governs coefficient
asymptotics of $G(z)$; the MFPT carries an additional Gaussian
dressing.  Carrying out the Laplace integral relating
$n \leftrightarrow V\rho_{\mathrm{peak}}$ on the exact birth-death
MFPT formula~\citep{Gardiner2009} gives
\begin{equation}\label{eq:schloegl-C-kramers}
  C_\infty =
  \frac{\pi}{w_+(x_a)\,\sqrt{\Phi''(x_a)\,|\Phi''(x_u)|}}
  \cdot \mathcal B(k_1,k_2,k_3,k_4),
\end{equation}
where $\Phi(x)$ is the quasi-potential and $\mathcal B$ is a
finite symbolic product from the reflecting wall at $n = 0$
(Gaveau--Moreau--Toth~\citep{GaveauMoreauToth1996}; vanishes when
$w_-(0) > 0$).  Both factors are rational/algebraic functions of
the rates via~\eqref{eq:schloegl-branchpoly}.  No stochastic
simulation is required to compute $\tau(V)$ at any $V$.

\paragraph{Numerical verification.}
Two parameter sets, chosen so $VS$ is measurable yet rare:
\begin{center}\small
\begin{tabular}{@{}lccccccc@{}}\toprule
 & $k_1$ & $k_2$ & $k_3$ & $k_4$
 & $S$ (branch-gap) & $S$ (WKB agreement) & $C_\infty$ \\ \midrule
A & 3.00 & 0.80 & 2.0833 & 0.2423 & $0.0801$ & 14 digits & $12.68$ \\
B & 3.00 & 0.80 & 1.4417 & 0.0687 & $0.1215$ & $< 2\cdot 10^{-17}$ & $26.31$ \\
\bottomrule\end{tabular}
\end{center}
Gillespie ($3{,}000$ trajectories/row) gives
$\log \tau_{\mathrm{sim}} = a + bV$ with fitted slope $b$ agreeing
with the symbolic $S$ to $\leq 5\cdot 10^{-3}$; intercept $a$
agrees with $\log C_\infty$ with a
$\sim 0.2$--$0.3$ shortfall consistent with $\mathcal O(1/V)$
corrections to the leading Laplace asymptotic (ratio
$\tau_{\mathrm{sim}}/(C_\infty \ee^{VS}) \to 1$ monotonically as
$V$ grows: $0.82, 0.90, 0.93, 0.87$ for set A, V=25--81).

\paragraph{Interpretation.}
Both ingredients of $\tau = C \ee^{VS}$ are now closed-form
algebraic functions of the reaction rates: $S$ from the branch-gap
integral via Cardano on the cubic DSE; $\mathcal K$ from
\eqref{eq:schloegl-K} directly; $C_\infty$ from the Kramers
asymptotic \eqref{eq:schloegl-C-kramers} built on the same
polynomial data.  The transfer-theorem amplitude $\mathcal K$ is
\emph{distinct} from the Kramers prefactor $C_\infty$---the former
is the coefficient of $(z_* - z)^{1/2}$ in $G(z)$; the latter is
$\tau/\ee^{VS}$---and the two are linked by a Laplace
change-of-variable.  The original ``weakness~(d)'' (prefactor
deferred to empirical calibration) is closed for polynomial
one-species CRNs; the script reproducing every quantity here is at
\path{wip/prefactor_C.py}.
...
(solutions of $\det(\widetilde L_F - \mu \widetilde L_P) = 0$).\footnote{The
term ``pencil'' comes from classical projective geometry, where a
\emph{pencil of conics} is the one-parameter family
$\lambda C_1 + \mu C_2 = 0$ of conics passing through the four
common intersection points of two fixed conics $C_1, C_2$---a
bundle of curves ``fanning out'' from a shared source (Latin
\textit{peniculus}, ``little brush'').  A \emph{matrix pencil} is
the same construction in linear algebra: the one-parameter family
$A - \lambda B$ formed from two fixed matrices, with the
\emph{generalised eigenvalue problem}
$\det(A - \lambda B) = 0$ asking where the family degenerates.
The standard eigenvalue problem is the special case $B = I$.
Matrix pencils and their generalised eigenvalue problem are
treated systematically in numerical linear
algebra~\citep{GolubVanLoan2013}; the QZ algorithm is the standard
diagonalisation method.  For the two-colour Kirchhoff polynomial,
the pencil of graph Laplacians $p \widetilde L_P + f \widetilde L_F$ is
parameterised by the two colour weights, and its generalised
eigenvalues are the roots that factor $\mathcal{K}(G; p, f)$
over~$\mathbb{R}$.}  Both reduced Laplacians are symmetric and
positive semidefinite, so every $\mu_i \geq 0$.
\end{theorem}
...
\subsection{Structural factorisation theorem for DP--MSR Keller--Segel}

We now strengthen the refined rationality claim from a conjecture
to a theorem, specifically for the DP--MSR Keller--Segel bacterial
self-energy.  The mechanism is the KS vertex signature $(\deg_P,
\deg_F) = (2, 1)$, which forces a graph-theoretic constraint on
the $F$-coloured subgraph that is strong enough to deliver a
rational pencil factor of known multiplicity.

\begin{lemma}[Perfect-matching structure]
\label{lem:matching}
Let $G$ be a 1PI DP--MSR Keller--Segel bacterial self-energy
graph at loop order $L \geq 1$ (two external $\psi$--$\tilde\psi$
legs), with any vertex-consistent two-colouring
$c: E_{\mathrm{int}} \to \{P, F\}$.  Then the $F$-coloured
spanning subgraph $H_F = (V, E_F)$ is a \emph{perfect matching}
on the $V = 2L$ internal vertices: every internal vertex has
$\deg_{H_F}(v) = 1$, and $|E_F| = L$.
\end{lemma}

\begin{proof}
The KS vertex signature assigns exactly one $F$-leg per internal
vertex; the two externals are both $P$.  Hence every internal
vertex has $\deg_{H_F}(v) = 1$ counting only internal edges.  So
$H_F$ is $1$-regular on $V$, a perfect matching; $|E_F| = V/2 = L$
by the leg-counting identity $3V = 2I + E_{\mathrm{ext}}$ with
$E_{\mathrm{ext}} = 2$.
\end{proof}

\begin{theorem}[Structural factorisation of the two-colour Kirchhoff, DP--MSR KS]
\label{thm:ksfactor}
Let $G$ be a 1PI DP--MSR KS bacterial self-energy graph at loop
order $L \geq 1$, with any vertex-consistent two-colouring.  Then
\begin{equation}\label{eq:pLfactor}
  \mathcal{K}(G; p, f)
  \;=\; p^{\,L-1}\cdot Q_G(p, f),
  \qquad
  Q_G \in \mathbb{Z}[p, f],\;\deg Q_G = L,
\end{equation}
with $Q_G$ a homogeneous binary form.  Equivalently, the
Laplacian pencil $(\widetilde L_F, \widetilde L_P)$ has the rational
generalised eigenvalue $\mu = 0$ with geometric multiplicity
$\geq L - 1$.  For $L \geq 2$, $\mathcal{K}(G; p, f)$ is therefore
reducible over $\mathbb{Q}$.
\end{theorem}

\begin{proof}
By Lemma~\ref{lem:matching}, $H_F$ has exactly $L$ connected
components (one per matched pair), so the Laplacian $L_F$ has
nullity $L$, with kernel spanned by the component-indicator
vectors $\mathbf 1_{C_1}, \ldots, \mathbf 1_{C_L}$ (which jointly
span $\mathbf 1 = \sum_i \mathbf 1_{C_i}$).  Deleting any one row
and column to form $\widetilde L_F$ quotients out $\mathbf 1$ and
leaves nullity $L - 1$, spanned by $L - 1$ independent
component indicators.  On this $(L-1)$-dimensional common kernel,
the pencil $p \widetilde L_P + f \widetilde L_F$ acts as $p \widetilde L_P$.
Expanding $\det(p \widetilde L_P + f \widetilde L_F)$ in a basis that
includes these null-vectors yields the structural factor $p^{L-1}$;
the remaining homogeneous binary form $Q_G(p, f)$ has degree
$(V - 1) - (L - 1) = L$.

For $L \geq 2$, the factor $p^{L-1}$ is non-trivial (degree
$\geq 1$) and strictly splits $\mathcal{K}(G; p, f)$, giving
$\mathbb{Q}$-reducibility.  The pencil eigenvalue $\mu = 0$ is
manifestly rational with multiplicity $L - 1$.  \qed
\end{proof}

Theorem~\ref{thm:ksfactor} is the concrete DP--MSR case of the
balance lemma at $(\alpha, \beta) = (0, 1)$: the KS vertex
signature forces $L_F$ to have nullity $L \geq 2$ automatically,
providing $L - 1$ independent non-constant vertex functions as
required by the lemma.  The conjectural form previously stated
here is thereby proved in the strong variant (not only one
rational eigenvalue, but $L-1$ of them).

\paragraph{Residual form $Q_G$: the remaining open question.}
Whether the degree-$L$ residual factor $Q_G(p,f)$ \emph{itself}
splits further over $\mathbb{Q}$ is topology- and colour-dependent.
At $L = 3$ (the $K_4 \setminus e$ topology) an exhaustive L=3
check gives $Q_G$ splitting on 254 of 256 colourings.  At $L = 4$,
enumeration of six representative topologies and all their
KS-consistent colourings (the perfect matchings of the internal
8-vertex graph, by Lemma~\ref{lem:matching}) shows a mixed
picture: some $Q_G$ fully split into linear factors (e.g.\
$K_{3,3}$-sub: $Q_G = 4f(f+p)(2f+p)(3f+4p)$); some split into
linear + irreducible cubic (e.g.\ cube minus edge:
$Q_G = (3f+p)(8f^3 + 20f^2 p + 11 f p^2 + p^3)$); and at least
one is irreducibly quartic over $\mathbb{Q}$ (prism-sub:
$Q_G = 20f^4 + 60 f^3 p + 54 f^2 p^2 + 16 f p^3 + p^4$).  The
``strong'' form of Conjecture~\ref{conj:twocolour}---all pencil
eigenvalues rational---therefore \emph{fails} at $L = 4$; the
refined form stated as Theorem~\ref{thm:ksfactor} (at least one
rational eigenvalue, with multiplicity $\geq L - 1$) holds
unconditionally.

\begin{conjecture}[Strong residual rationality, refined]
\label{conj:twocolour}
For 1PI KS bacterial-self-energy graphs, there is no further
closed-form splitting of $Q_G$ over $\mathbb{Q}$ beyond the
cases identifiable by explicit balance-lemma witnesses
$(\alpha, \beta) \neq (0, 1)$.  In particular, the residual
rationality class depends on graph invariants of $Q_G$, which we
leave as a computer-algebra classification problem.
\end{conjecture}

\paragraph{Balance lemma (sufficient condition, now a corollary).}
...
The DP--MSR case Theorem~\ref{thm:ksfactor} is the
$(\alpha, \beta) = (0, 1)$ instance where the KS vertex signature
automatically provides $L - 1$ independent balance witnesses.
Other balance pairs $(\alpha, \beta)$ can produce additional
rational factors for specific graphs and colourings; these
account for the further rational splittings of $Q_G$ observed
empirically at $L = 3, 4$.
...
Theorem~\ref{thm:ksfactor} strengthens this for DP--MSR KS: the
bridge table decomposes explicitly into an $(L-1)$-fold
$P$-monochromatic piece plus a residual $Q_G$-spectrum of degree
$L$, shrinking the effective bridge-table entries by roughly half
at each loop order.  The two colour sectors not only do not mix
irreducibly at the level of Schwinger integrals, they decouple
structurally by a margin proportional to the loop order.
...
Theorem~\ref{thm:ksfactor} (bi-colour structural factorisation)
gives a cleaner, system-independent computational route.  The
...
Theorem~\ref{thm:ksfactor} is a statement about the matrix-tree
theorem on bi-coloured graph Laplacians.  It establishes the
$p^{L-1}$ structural factor for all $L \geq 2$, verified
numerically through $L = 4$; the residual form $Q_G(p,f)$ may or
may not split further over $\mathbb{Q}$, a topology-dependent
arithmetic question (Conjecture~\ref{conj:twocolour}).  The
effective consequence is that CFAC's bi-colour bridge table has
an $(L-1)$-fold monochromatic piece plus a degree-$L$ residual,
generalising the single-colour results
...
\subsection{Contributions, distilled}
\label{sec:contributions}

It is worth stating plainly what this paper adds and what it does
not.  The paper does \emph{not} compute a new critical exponent,
solve a long-standing open problem in active matter, or
out-compute the direct-RG programme on its own terms.  What it
does is restructure how coupled particle-field calculations are
\emph{organised}, in a way that makes several objects algebraic
that were previously obtained by case-by-case computation.

Five concrete contributions, in order of structural depth:
\begin{enumerate}[leftmargin=1.8em,itemsep=0.15em]
\item \textbf{The recognition theorem} (companion note): the
  Doi--Peliti/MSR dressed-propagator generating function of any
  CRN-in-medium is a Lagrange equation $G = z \phi(G)$ in the
  sense of Flajolet--Sedgewick.  The proof is a three-line
  rewriting of the Dyson--Schwinger equation; its \emph{content}
  is that a body of mathematics not previously drawn on in
  stochastic field theory---the transfer theorem, Puiseux
  classification of universality, Newton-polygon singularity
  analysis---transfers wholesale.  Everything else in this paper
  is a consequence of this recognition being applied to
  coupled-field examples.
\item \textbf{The branch-gap contour integral} for the
  nucleation barrier (Theorem~\ref{thm:gap}) puts perturbative
  (C1) and non-perturbative (C3) physics on the same
  singularity of the same generating function.  Direct-RG
  handles the two with distinct technology (Feynman diagrams for
  perturbation theory, complex-field saddle points for
  instantons); CFAC unifies them.  The moderate-coupling
  extension via complex-conjugate saddle sum
  (Prop.~\ref{prop:complex-branchgap}) pushes this through to
  the regime where the real algebraic formulation fails.
\item \textbf{The matrix-tree pencil theorem} for the two-colour
  Kirchhoff polynomial (Theorem~\ref{thm:pencil}), together with
  the DP--MSR KS structural factorisation
  (Theorem~\ref{thm:ksfactor}).  $\mathcal{K}(G; p, f) =
  p^{L-1}\,Q_G(p, f)$ at every loop order, forced by the
  perfect-matching structure of the field-coloured subgraph.
  This is the one place the paper states a theorem about
  coupled-field diagrams that was previously only conjectural and
  numerically verified.  It is the technical centrepiece.
\item \textbf{The substitution table for fractal substrates}
  (Prop.~\ref{prop:substitution-table}).  Different physical
  exponents inherit different dimensions from a non-Euclidean
  substrate: universality class is substrate-independent,
  kinetic exponents use $d_s$, CNT droplet uses $d_f$, walk
  statistics use $d_w$.  The row-by-row decomposition is
  apparently new in this clean form; Row~4
  $\bigl(\Phi_c \sim \sigma^{d_f}/\Delta f^{d_f - 1}\bigr)$ is
  the apparently-novel prediction, modulo an important
  ramification caveat.
\item \textbf{The one-loop tropical bridge}
  (Prop.~\ref{prop:tropical-exact}) gives the one-loop integral
  in closed algebraic form (AHHS leading-facet+Hessian), exact
  because the Symanzik polynomial is quadratic at one loop.
  Combined with the one-loop-exactness criterion
  (Prop.~\ref{prop:one-loop-criterion}) this makes the entire
  CFAC pipeline closed-form symbolic end-to-end for the CRNs of
  physical interest (pair annihilation, pure DP, BARW,
  chemotaxis, KS).
\end{enumerate}

\subsection{What we find most interesting}
\label{sec:interesting}

An honest ranking.  The \textbf{branch-gap contour integral} is
the single result we expect physicists to care about most: a
non-perturbative rate from the same polynomial that controls the
perturbation series, computed by contour integration around a
branch point rather than by complex-field saddle-point.  The
complex-conjugate extension to moderate coupling makes it robust
beyond weak coupling.

The \textbf{DP--MSR bi-colour structural theorem} is the result
we find most mathematically satisfying: a graph-theoretic
theorem that the KS vertex signature $(2, 1)$ forces a perfect
matching on the field-coloured subgraph, giving a pencil
eigenvalue of multiplicity $L - 1$.  The mechanism is elementary
once seen but wasn't seen before; its consequence---that
bi-colour bridge integrals decompose with an $(L-1)$-fold
monochromatic factor---says the two colour sectors decouple
structurally at every loop order.

The \textbf{substitution table} is the result we think is most
likely to propagate into other calculations.  Once stated as a
row-by-row decomposition---with each row of the CFAC pipeline
picking up exactly one substrate dimension---it becomes a
template anyone working on RD systems on complex substrates can
use.  Row 4 (CNT $\sim d_f$) is the prediction we'd most like to
see tested cleanly.

The \textbf{one-loop tropical bridge} is the result we think is
most under-appreciated.  ``The one-loop bridge has a closed
algebraic form'' sounds technical; what it \emph{means} is that
for one-loop-exact CRNs (the standard case) the whole CFAC
pipeline---from reaction network to beta function---is closed in
polynomial algebra, with no numerical quadrature, table lookup,
or Monte Carlo anywhere.  This sharpens what ``CFAC factors
counting out of coupled particle-field renormalisation'' means.

\subsection{Future work}
\label{sec:future}

Four concrete directions, in decreasing order of confidence that
they are tractable and worthwhile.

\paragraph{A second coupled system: the ant/KPZ pipeline.}
Everything in this paper has been demonstrated on KS; a second
coupled example in a \emph{different universality class} is the
natural next step.  Our primary candidate is a chemotactic ant
colony---foragers laying a volatile pheromone trail (MSR field)
while a discrete population (DP) senses its gradient---because
in the continuum limit the trail field is described by the
Kardar--Parisi--Zhang equation~\citep{KPZ1986} and the system
inherits a non-trivial Puiseux exponent $p/q = 1/3$ rather than
the mean-field $1/2$ of KS.  Specifically, we expect CFAC to
reproduce the critical recruitment density
$\lambda_c \sim \rho^{1/3}$ (observed in pheromone-trail
experiments on \textit{L.\ niger} and \textit{M.\ sanguinea}; see
Deneubourg \& Goss, \textit{J.\ Theor.\ Biol.}\ 1989) from the
vertex dictionary alone, exercising three parts of the framework
that KS does not: (i)~the compact-DP / triple-root Puiseux class
in (C2); (ii)~a KPZ-type $(\nabla\phi)^2$ vertex, which has
$\tau_v = 0$ and therefore sits at the critical edge of
Proposition~\ref{prop:one-loop-criterion}'s one-loop-exactness
condition; (iii)~Row~4 of the substitution table in a system
where the nucleation event is collective (colony-level
switching) rather than local (ice-crystal initiation), testing
whether $\Phi_c \sim \sigma^{d_f}/\Delta f^{d_f - 1}$ carries the
right interpretation for a sociobiological transition.  A full
writeup ties the CFAC paper into the larger AC+ programme and
gives a second physical system on which the pipeline is
demonstrated end-to-end.

\paragraph{A second coupled system from the literature: Toner--Tu.}
For readers who prefer a target whose one-loop RG is already
published, the Toner--Tu theory of polar
flocking~\citep{TonerTu1995,TonerTu1998,Toner2012} is the
cleanest choice.  It is a DP--MSR coupled action in exactly the
sense used here: self-propelled particles with number
fluctuations (DP) coupled to a velocity and density field
(MSR), with an explicit one-loop calculation culminating in the
anomalous dynamical exponent $z = 2$ and the spectacular
Mermin--Wagner-evading long-range order in $d = 2$.  A CFAC
re-derivation would exercise the vertex dictionary in a
different way: convective $\psi \nabla \psi$ vertices give
$\tau_v > 0$, so the one-loop-exactness criterion
(Proposition~\ref{prop:one-loop-criterion}) does \emph{not}
apply---Toner--Tu flocking is a genuine case where higher-loop
UV divergences persist and the tropical bridge loses its closed
form.  It is therefore the right stress test for the scope of
the framework.  The two-colour Kirchhoff pencil
(Theorem~\ref{thm:pencil}) still applies; whether the
DP--MSR-KS-style structural factor extends to flocking is an
open question whose answer would immediately generalise the
perfect-matching argument.  Active nematic
systems~\citep{RamaswamySimhaToner2003,ChenToner2013} generalise
this further and have similar published one-loop anchors.

\paragraph{CNT droplet scaling on an infinitely-ramified fractal.}
The substitution-table prediction
$\Phi_c \sim \sigma^{d_f}/\Delta f^{d_f - 1}$ is honest for
substrates where $|\partial B(R)| \sim R^{d_f - 1}$, which holds
on Sierpinski carpets but fails on gaskets.  A clean
verification requires a carpet substrate (say $d_f = \log 8/\log
3 \approx 1.893$, predicted CNT slope $-0.893$) at weak
inter-site coupling.  We expect the prediction to hold; a clean
numerical test would turn Row~4 from ``derivation'' into
``derivation plus verification''.

\paragraph{Complete Galois/monodromy analysis of the coupled resultant.}
The moderate-coupling branch-gap extension via complex-conjugate
saddles
(Prop.~\ref{prop:complex-branchgap}) empirically finds three
pairs at $\alpha = 0.3$ on the 3-site ring; a general proof that
the reachable conjugate-pair set is always finite and
combinatorially determined from the resultant's algebraic data
would upgrade the proposition to a theorem.  This is a question
in computational algebraic geometry---the complex Riemann
surface of a two-variable polynomial---and is a tractable PhD
project.

\paragraph{Matrix-tree beyond one-loop and beyond two colours.}
Theorem~\ref{thm:ksfactor} covers DP--MSR KS (bacterial
self-energy).  Chemical-sector self-energies follow by symmetry
with the $F$/$P$ roles swapped.  Mixed-sector $n$-point functions
(one $\psi$ external, one $\phi$ external) are not covered---the
perfect-matching argument relies on both externals being the
same colour.  The natural extension is a matrix-tree statement
for graphs with mixed colour externals, likely involving a
second Symanzik polynomial $F(G; p, f)$ in place of $\mathcal K$.
More speculatively: three-colour generalisations for theories
with two fields plus a reservoir (a DP--MSR--MSR system, e.g.\
bacteria + chemoattractant + oxygen) would give three-variable
pencils whose factorisation structure is unknown.  The
matrix-tree technology extends but the specific ``perfect
matching forces factorisation'' mechanism does not generalise
mechanically; new structural observations are needed.

Beyond these four: a functional-analytic extension of the
recognition theorem itself (to momentum-dependent vertex
couplings, giving a Lagrange equation in function space rather
than formal power series) would cover the chemotaxis coupling at
full momentum without the zero-momentum projection; higher-loop
tropical bridges (beyond one loop, where the Symanzik is no
longer quadratic) need Borinsky's multi-facet technology and
remain algorithmic rather than closed-form.  These are real but
distinctly harder.
...
$d > 0$ in literal Theorem~\ref{thm:gap} form.  The real-algebraic
formulation extends verbatim only in the decoupled limit
(agreement to 2.6\% on a 3-site test), under-predicts by
$\sim 20$\% at weak inter-site coupling, and loses its real
branch points entirely at moderate coupling.  The refined form
(Proposition~\ref{prop:complex-branchgap}) extends to moderate
coupling by complex analytic continuation of the resultant with
a Coleman--Callan sum over complex-conjugate saddle pairs; this
achieves $\sim 3\%$ agreement with Gillespie at $\alpha = 0.3$,
but lacks a general proof that the reachable conjugate-pair set
is always finite and combinatorially enumerable from the
resultant (a full Galois/monodromy question).  The paper also
does not (b) prove the strong form of
Conjecture~\ref{conj:twocolour} (full $\mathbb{Q}$-splitting of
the residual $Q_G$ in Theorem~\ref{thm:ksfactor}); an $L = 4$
counter-example (prism-sub with irreducible quartic $Q_G$) is
given, so the strong form does \emph{not} hold; the theorem
$(p^{L-1} \mid \mathcal{K})$ is the unconditional statement we
establish.  (c) extrapolate the one-loop result to
$\varepsilon = 1$ quantitatively, where the $\varepsilon$-expansion
is known to fail for one-loop approximations~\cite{Tauber};
(d) the prefactor $C$ in $\tau = C\,\ee^{VS}$ is closed in the
worked Schl\"ogl-I example (\S\ref{sec:prefactor-schloegl}) both
as transfer-theorem amplitude $\mathcal K$ and as Kramers prefactor
$C_\infty$; extending the symbolic Kramers calculation to coupled
systems beyond the adiabatic-field reduction is the residual
open question; (e) handle CRNs for which
...
\bibitem[Gardiner(2009)]{Gardiner2009} Gardiner, C., 2009. \textit{Stochastic Methods: A Handbook for the Natural and Social Sciences}, 4th ed., Springer.
\bibitem[Golub and Van Loan(2013)]{GolubVanLoan2013} Golub, G.~H.\ and Van Loan, C.~F., 2013. \textit{Matrix Computations}, 4th ed., Johns Hopkins Univ.\ Press.
\bibitem[Gaveau et al.(1996)]{GaveauMoreauToth1996} Gaveau, B., Moreau, M., and Toth, J., 1996. Decay of the metastable state: different predictions between discrete and continuous descriptions. \textit{J.\ Stat.\ Phys.} 85, 419.
\bibitem[Kardar et al.(1986)]{KPZ1986} Kardar, M., Parisi, G., and Zhang, Y.-C., 1986. Dynamic scaling of growing interfaces. \textit{Phys.\ Rev.\ Lett.} 56, 889.
\bibitem[Toner and Tu(1995)]{TonerTu1995} Toner, J.\ and Tu, Y., 1995. Long-range order in a two-dimensional dynamical XY model: how birds fly together. \textit{Phys.\ Rev.\ Lett.} 75, 4326.
\bibitem[Toner and Tu(1998)]{TonerTu1998} Toner, J.\ and Tu, Y., 1998. Flocks, herds, and schools: a quantitative theory of flocking. \textit{Phys.\ Rev.\ E} 58, 4828.
\bibitem[Toner(2012)]{Toner2012} Toner, J., 2012. Reanalysis of the hydrodynamic theory of fluid, polar-ordered flocks. \textit{Phys.\ Rev.\ E} 86, 031918.
\bibitem[Ramaswamy et al.(2003)]{RamaswamySimhaToner2003} Ramaswamy, S., Simha, R.~A., and Toner, J., 2003. Active nematics on a substrate: giant number fluctuations and long-time tails. \textit{Europhys.\ Lett.} 62, 196.
\bibitem[Chen and Toner(2013)]{ChenToner2013} Chen, L.\ and Toner, J., 2013. Universality for moving stripes: a hydrodynamic theory of polar active smectics. \textit{Phys.\ Rev.\ Lett.} 111, 088701.
