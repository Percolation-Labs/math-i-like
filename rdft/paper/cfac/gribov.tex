...
\texttt{run\_all\_tests.py} executes 10 end-to-end tests covering
...
identification, the causal$\to$Euclidean reduction, and a
contraction-view honesty check --- \textbf{all 10 pass} on the
development machine.
...
Below we determine all 12 by closure with JT05 Eq.~(57) plus 5
structural CFAC constraints; the parallel a-priori derivation via
\texttt{OmegaIntegration}~$+$~\texttt{ibp\_solver} has its components
independently verified (tests \#6--9) and is open as glue work.
...
\begin{codebox}[End-to-end test suite: 10/10 passing]
...
linear system by Laporta priority sort.  Tests 9--10 verify the
causal$\to$Euclidean reduction (graph-polynomial substitutions on
Schwinger parameters) and explicitly demonstrate that the output is
the parametric integrand of the code, not the abstract contracted
graph.  Nothing numerical, nothing opaque.
...
10 & Contraction-view honesty check: code output vs.\ literal tadpole & \checkmark\\
\midrule
\multicolumn{2}{r}{\bfseries Summary} & \textbf{10/10 pass} \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{(D1) End-to-end IBP wiring for Reggeon DP.}  Both
    modules already verified independently (test \#9: causal
    reduction on bubble + sunset; tests \#6--8: Laporta solver on
    standard massless sunset).  The remaining glue: pipe the output
    of \texttt{OmegaIntegration} on each Reggeon 2-loop topology into
    \texttt{ibp\_solver} as input, run Laporta triangulation, read
    off the $q^X_\bullet$ rationals.  No theoretical content; pure
    plumbing.
  \item \textbf{(D2) Algebra factors $a_\Gamma$.}  Enumerate the
...
is labelled test \#9 of \texttt{run\_all\_tests.py} (10/10 passing).
Test \#10 cross-checks honestly that this output is \emph{not} the
Symanzik of an abstract contracted graph (the topological
``$G//P$'' picture is heuristic, not literal --- see
\Cref{app:where-ibp-enters} for the side-by-side comparison).
\end{codebox}
...
$Z_\psi$-row IBP coefficients $q^\psi_{\textsf{sun}}, q^\psi_{B_2^2}$
  & JT05-closure (each step of a-priori route independently verified, end-to-end wiring open)
  & \texttt{OmegaIntegration} verified on bubble + sunset (test \#9); \texttt{ibp\_solver} verified on standard massless sunset (tests \#6--8); the chain of the two for Reggeon-DP topologies is the open glue step.  See \Cref{app:where-ibp-enters} \\
