...
\subsubsection{Pictures: bubble}

\begin{cfacbox}[Reggeon bubble $\to$ Euclidean bubble]
\begin{center}
\begin{tikzpicture}[font=\footnotesize, baseline]
  % Left: Reggeon bubble with frequency-flow arrows
  \node[circle, fill=black, inner sep=1.2pt] (l1) at (-5.0, 0) {};
  \node[circle, fill=black, inner sep=1.2pt] (l2) at (-3.0, 0) {};
  \draw[thick, ->] (l1) to[bend left=60] node[above]{$\alpha_0,\,\omega$} (l2);
  \draw[thick, <-] (l1) to[bend right=60] node[below]{$\alpha_1,\,\omega$} (l2);
  \draw[thick] (-5.7, 0) -- (l1);
  \draw[thick] (l2) -- (-2.3, 0);
  \node at (-4.0, -1.4) {\textbf{Reggeon bubble}};
  \node at (-4.0, -1.85) {\scriptsize causal: $\dd\omega/(2\pi) \int$};

  % Arrow with delta-function
  \draw[->, very thick, draw=cfacline]
       (-1.9, 0) -- node[above]{\scriptsize $\delta(\alpha_1 - \alpha_0)$}
                  node[below]{\scriptsize \emph{contour}}
       (0.3, 0);

  % Right: Euclidean bubble with identified Schwinger parameter
  \node[circle, fill=black, inner sep=1.2pt] (r1) at (1.0, 0) {};
  \node[circle, fill=black, inner sep=1.2pt] (r2) at (3.0, 0) {};
  \draw[thick] (r1) to[bend left=60] node[above]{$\alpha_0$} (r2);
  \draw[thick] (r1) to[bend right=60] node[below]{$\alpha_0$} (r2);
  \draw[thick] (0.3, 0) -- (r1);
  \draw[thick] (r2) -- (3.7, 0);
  \node at (2.0, -1.4) {\textbf{Euclidean bubble}};
  \node at (2.0, -1.85) {\scriptsize $\Psi = 2\alpha_0$};
\end{tikzpicture}
\end{center}
The two edges' Schwinger parameters get \emph{identified} ($\alpha_0
= \alpha_1$) by the delta function $\delta(\alpha_1 - \alpha_0)$
arising from the single loop's frequency integration.  The reduced
Symanzik polynomial is $\Psi = \alpha_0 + \alpha_1\big|_{\alpha_1 =
\alpha_0} = 2\alpha_0$, which is the ``one effective Schwinger
parameter with multiplicity 2'' that a relativistic IBP would see.
\end{cfacbox}

\subsubsection{Pictures: sunset}

\begin{cfacbox}[Reggeon sunset $\to$ diagonal Euclidean]
\begin{center}
\begin{tikzpicture}[font=\footnotesize, baseline]
  % Left: Reggeon sunset (3 parallel propagators between 2 vertices)
  \node[circle, fill=black, inner sep=1.2pt] (sl) at (-5.0, 0) {};
  \node[circle, fill=black, inner sep=1.2pt] (sr) at (-3.0, 0) {};
  \draw[thick, ->] (sl) to[bend left=70] node[above]{$\alpha_0,\,\omega_1$} (sr);
  \draw[thick, ->] (sl) to node[above, midway]{$\alpha_1,\,\omega_2$} (sr);
  \draw[thick, ->] (sl) to[bend right=70] node[below]{$\alpha_2,\,\omega_3$} (sr);
  \draw[thick] (-5.7, 0) -- (sl);
  \draw[thick] (sr) -- (-2.3, 0);
  \node at (-4.0, -1.5) {\textbf{Reggeon sunset}};
  \node at (-4.0, -1.95) {\scriptsize 2 loops; 2 frequency ints};

  % Arrow
  \draw[->, very thick, draw=cfacline]
       (-1.9, 0) -- node[above]{\scriptsize $\delta(\alpha_1{-}\alpha_0),\,\delta(\alpha_2{-}\alpha_0)$}
       (0.3, 0);

  % Right: Euclidean diagonal sunset (all three alphas identified)
  \node[circle, fill=black, inner sep=1.2pt] (esl) at (1.0, 0) {};
  \node[circle, fill=black, inner sep=1.2pt] (esr) at (3.0, 0) {};
  \draw[thick] (esl) to[bend left=70] node[above]{$\alpha_0$} (esr);
  \draw[thick] (esl) to node[above, midway]{$\alpha_0$} (esr);
  \draw[thick] (esl) to[bend right=70] node[below]{$\alpha_0$} (esr);
  \draw[thick] (0.3, 0) -- (esl);
  \draw[thick] (esr) -- (3.7, 0);
  \node at (2.0, -1.5) {\textbf{Euclidean sunset}};
  \node at (2.0, -1.95) {\scriptsize $\Psi = 3\alpha_0^2$};
\end{tikzpicture}
\end{center}
Two delta functions (one per loop) identify all three Schwinger
parameters: $\alpha_0 = \alpha_1 = \alpha_2$.  The reduced Symanzik
polynomial is $\Psi = (\alpha_0 \alpha_1 + \alpha_0 \alpha_2 +
\alpha_1 \alpha_2)\big|_{\alpha_1 = \alpha_2 = \alpha_0} = 3\alpha_0^2$
--- exactly the diagonal contraction of the textbook 2-loop sunset
Symanzik.
\end{cfacbox}
...
\subsubsection{Why this works: the broadcasting principle}

\begin{stdbox}[Topological interpretation, after thesis Remark 32]
Each $\delta$-substitution can be read in two equivalent ways:
\begin{itemize}[leftmargin=2em]
  \item \textbf{As a graph contraction $G //\!P$}: the propagator $P$
    that carried the residue pole is contracted (its endpoints
    identified, its edge removed/identified with its parallel
    edges).
  \item \textbf{As kinematics broadcasting}: any external momentum or
    mass $\pm i\omega_l$ that flowed on $P$ is ``broadcast'' to the
    parallel edges, so the residue value depends only on what
    survives on the contracted graph.
\end{itemize}
The bubble: contracting either edge gives a single line with the sum
of the two masses.  The sunset: contracting two of the three edges
gives a single line with the sum of the three masses.

The Reggeon-graph perspective and the Euclidean-graph perspective are
related by this contraction; the rationals on the IBP-reduction side
do not know whether the input was Reggeon-causal or
relativistic-Euclidean, only that they got a Symanzik polynomial of
a specific structure.  This is why a single IBP solver can serve
both worlds.
\end{stdbox}
