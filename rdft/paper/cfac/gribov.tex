...
The function \texttt{lagrange\_count} in
\texttt{rdft/ac/ibp\_solver/sunset\_full.py} (or equivalently
\texttt{assembly.py}) computes:
\begin{verbatim}
  >>> [z^1] G(z)  =  1
  >>> [z^3] G(z)  =  1
  >>> [z^5] G(z)  =  2
  >>> [z^7] G(z)  =  5
\end{verbatim}
verifying the pen-and-paper output above.
\end{codebox}
...
\subsubsection{Primitive sunset residue (master input)}
...
\[
  ...
  \qquad L = \ln(4/3).
\]
For $Z_\psi$ extraction:
\[
  \mathrm{prim}_\psi
  \;=\; q^\psi_{\textsf{sun}}\,(\text{sun residue})
        + q^\psi_{B_2^{\,2}}\,(B_2^{\,2}\text{ residue}).
\]
With our normalisation (\Cref{sec:simple-pole-closure}) this evaluates
to $\mathrm{prim}_\psi = 1/16 + (9/64)\,L$.
\end{stdbox}
...
\emph{checked} by the code; they are \emph{the same operations}.  The
Lagrange-inversion polynomial expansion at $n = 5$ is exactly what
\texttt{lagrange\_count(5, 1+G**2)} evaluates.  The Hopf-antipode
double-pole identity is what \texttt{(1/2) * a[X] * (beta\_1 + a[X])}
in the script computes.  The script is the pen-and-paper, mechanised.
