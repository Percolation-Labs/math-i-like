...
\begin{stdbox}[Honest status of the IBP step in this walkthrough]
The $q^\psi_\bullet$ rationals played the role IBP plays in the
standard 2-loop calculation: they are the linear-algebra coefficients
that decompose a generic 1PI 2-point amplitude (after BPHZ) onto the
master basis.  In a fully a-priori IBP derivation one would:
\begin{enumerate}[leftmargin=2em]
  \item Set up the Reggeon-causal sunset propagator family
    $D_i = -i\omega_i + \lambda(\tau + p_i^2)$.
  \item Generate IBP identities $\int \partial_{k_j}^\mu (v^\mu /
    D_1^{a_1}\cdots) = 0$ with all numerator structures relevant to
    $\partial_{(-i\omega)}$ extraction.
  \item Solve the resulting linear system over $\mathbb{Q}(d, \omega,
    p^2, \tau)$ and Laporta-triangulate down to the 3-master basis.
  \item Read off $q^\psi_{\textsf{sun}}$, $q^\psi_{B_2^{\,2}}$,
    $q^\psi_V$ as the rational coefficients in front of each master.
\end{enumerate}
Our from-scratch IBP solver in \texttt{rdft/ac/ibp\_solver/} does
exactly this for the \emph{standard massless} sunset (verifying the
textbook $(d-3)$ Laporta coefficient and identifying $I[1,1,1]$ as
the master).  It has \emph{not} been re-run with Reggeon causal
propagators in this paper.

Instead, in \Cref{sec:simple-pole-closure} we determined the
$q^X_\bullet$ rationals by combining 5 structural CFAC constraints
(\textsf{S1}--\textsf{S5}: which masters can feed which extraction,
which $L$-dependences are forbidden) with consistency against JT05
Eq.~(57)'s simple poles.  This is a \emph{closure} argument, not an
a-priori IBP derivation.  It establishes that JT05's numbers are
consistent with the framework's structure, but it does not
independently produce the $q$'s from IBP alone.

For the present paper this is a transparent gap: the $Z_\psi$-row
$\{9/64, 1/16, 0\}$ above came from JT05-closure, not from running
the IBP solver on Reggeon kinematics.  Closing the gap is mechanical
(propagator substitution in the existing solver + a Laporta-style
solve) and is listed in \Cref{sec:open}.
\end{stdbox}
...
$Z_\psi$-row IBP coefficients $q^\psi_{\textsf{sun}}, q^\psi_{B_2^2}$
  & JT05-closure (NOT a-priori IBP)
  & See \Cref{app:where-ibp-enters}; would close by re-running our \texttt{ibp\_solver} on Reggeon-causal propagators \\
...
\begin{itemize}[leftmargin=2em,itemsep=1pt]
  \item The 12 IBP coefficients $q^X_\bullet$ are determined by
    \emph{closure with JT05}, not by running an IBP solver on
    Reggeon-causal propagators.  The from-scratch solver in
    \texttt{rdft/ac/ibp\_solver/} works on the standard massless
    sunset; the Reggeon extension is mechanical
    (propagator substitution) and not yet executed.
