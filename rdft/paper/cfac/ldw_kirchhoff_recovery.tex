...
diagram classes that LDWC eliminate over ${\sim}4$ pages of appendices.
...
\begin{description}
  ...
  \item[Class C (tadpole-containing).]
    ...
    single vertex).
\end{description}
LDWC show by direct computation over ${\sim}4$ pages of appendices
that only class A contributes new 2-loop structure; classes B and C
either cancel or combine into the one-loop counterterm.  We give the
CFAC derivation: both cancellations are consequences of two general
graph-theoretic results, not of LDWC-specific physics.
...
$\alpha=1$), the amplitude that multiplies $J_{2+3}$ carries an
extra factor that breaks the $\beta \leftrightarrow 1/\beta$
symmetry.  The $-\ln 2$ does not appear, leaving
$X^{(1)} = \ln 2$ (transcendental, but still determined by
$\Psi_{\text{sun}}$, not a free ODE integral).
...
\begin{itemize}
  \item The Kirchhoff--Symanzik pipeline correctly classifies all
    diagrams at two loops: two factorise (dbl-bubble, tadpole),
    one is irreducible but becomes rational after BPHZ + sector decomposition of $J$
    (sunset).
  \item The first Symanzik polynomial $\Psi_{\text{sun}}$
    \emph{is} the denominator in the LDWC integral $J$.
    LDWC compute $J$ by splitting at $\beta=1$; CFAC identifies why
    that split works: $\beta=1$ is the symmetry point of $\Psi_{\text{sun}}$.
    This is a structural identification, not a coincidence.
  \item The analytic proof $J_1 = \ln 2$ is derived from
    $\Psi_{\text{sun}}$, not assumed from LDWC.
  \item The residual count $\Nres = 1$ is a precise claim:
    exactly one 1D integral ($\gamma$) is needed; everything else
    is algebra.
\end{itemize}

\paragraph{What is taken from LDWC.}
\begin{itemize}
  \item $J_2 = -\ln 2$: the change-of-variables argument is standard
    but not rederived here.
  \item $J_3|_{\rm finite} = 1$: this requires dimensional
    regularisation of the $\beta\in[1,\infty)$ UV region.
    We accept LDWC's result; independent rederivation from our
    $\Psi_{\text{sun}}$ is straightforward but deferred.
  \item The prefactor $1/(9\sqrt{2})$: derived in full in
    Section~\ref{sec:assembly}.  Both factors of $3$ in $9=3^2$ come
    from the one-loop roughness $\zeta_1 = 1/3$; they are not from
    two-loop diagram topology.
  \item $J_2 = -\ln 2$: derivable from the $\beta\leftrightarrow 1/\beta$
    symmetry of $\Psi_{\text{sun}}$.  The explicit step-by-step
    computation by LDWC integrates $\beta$ first and obtains the
    same result through partial fractions and logarithmic
    cancellations~\citep[eq.~A.16]{LDWC2002}.
\end{itemize}

\paragraph{Open: complete independence.}
A fully self-contained derivation would compute $J_2$ and
$J_3|_{\rm finite}$ from $\Psi_{\text{sun}}$ using the
Schwinger parametric machinery developed here.
The integrals are tractable (they reduce to 1D quadratures)
and the result would close the gap.
...
\paragraph{Short-range case: $\Nres = 1$, $X^{(1)} = \ln 2$.}
The period integral of $1/\Psi_{\text{sun}}$ over the standard
simplex is $\ln 2$ --- a single transcendental, still in the
residual list but now topological.  Computing it from
$\Psi_{\text{sun}}$ directly (without quoting LDWC) is a one-line
extension of Proposition~\ref{prop:J1}.
