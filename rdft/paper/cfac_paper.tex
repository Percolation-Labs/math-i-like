...
reflects the ingredients: an algebraic formalism for coupled-field
...
\paragraph{What $\omega$ is doing.}
The superficial degree of divergence $\omega = dL - 2I +
n_\partial^{\mathrm{tot}}$ is standard QFT power
counting~\citep{Tauber}: a loop diagram in $d$ dimensions has
$dL$ momentum-integration powers, $-2I$ from propagator falloff
at large $k$, and $+n_\partial^{\mathrm{tot}}$ from gradient
insertions.  $\omega \geq 0$ means UV divergent (contributes to
renormalisation); $\omega < 0$ means UV finite.  We apply this
formula to each enumerated 1PI graph.
...
% ══════════════════════════════════════════════════════════════════
\section{Spectral substitution for coupled systems}
\label{sec:spectral}
% ══════════════════════════════════════════════════════════════════

Reaction-diffusion exponents on an irregular substrate follow
from $d \to d_s$~\cite{AlexanderOrbach,Havlin,Hattori}.  For
coupled systems with $D_A \neq D_c$, one could worry that
different fields see different effective substrates.

\begin{conjecture}[Universality of $d_s$]
\label{conj:ds}
Both fields see the same $d_s$; the substitution applies
uniformly to both sectors.
\end{conjecture}
...
\end{tabular}
\end{center}
All within 2\% of the exact value and independent of $D$, as
expected for a graph-intrinsic property.  (Finite-size effects
are significant: the same measurement at level 4, with only 123
vertices, gives apparent $d_s \in \{0.14, 0.49\}$---recovery of
$d_s$ requires a graph large enough that the random walk samples
