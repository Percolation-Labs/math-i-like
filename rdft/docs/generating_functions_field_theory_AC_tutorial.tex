...
on $G$. Specifically:
\begin{align*}
  \sum_{s,i} s\, P(s,i) x^s y^i &= x\,\partial_x G,\\
  \sum_{s,i} (s+1) P(s+1, i-1) x^s y^i &= \partial_x G \cdot \frac{y}{1} \cdot \ldots
\end{align*}

After careful bookkeeping (multiplying each term in \eqref{eq:master_sir} by $x^s y^i$
and summing), the master equation becomes a \textbf{single PDE}:
...
\begin{equation}
  T_* = \frac{1}{R_0}, \qquad z_* = \frac{1}{R_0} e^{1 - 1/R_0}.
...
GF singularity & $z_* = e^{1-1/R_0}/R_0$ & IFT failure \\
...
$z_* = (1/R_0)e^{R_0(1/R_0 - 1)} = (1/R_0)e^{1-1/R_0}$, confirming
...
$A + A \to A$ & 2 & $d/2$ & 1 \\
$A + B \to \emptyset$ & 4 & $d/4$ & 1/2 \\
$3A \to \emptyset$ & 1 & $d/2$ & 1 \\
\bottomrule
\end{tabular}
\end{center}
