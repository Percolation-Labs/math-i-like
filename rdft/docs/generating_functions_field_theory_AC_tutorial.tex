...
The variables $x$ and $y$ in the characteristic equations \eqref{eq:sir_chars} are
PGF dummy variables (conjugate to $S$ and $I$), not physical quantities. The
substitution $x = S/N$, $y = I/N$ is used only as a motivating identification.
What is exact is that the characteristic curves of the PGF PDE have the same
\emph{functional form} as the deterministic SIR ODEs \eqref{eq:sir_ode}:
the $\dot{x}$-equation along a characteristic is $\dot{x} = -(\beta/N)x(y-1)$,
and the $\dot{y}$-equation similarly matches $\dot{I}/N$ under the correspondence
$x \leftrightarrow S/N$, $y \leftrightarrow I/N$. No large-$N$ or small-$I/N$
approximation is needed; the equality is an identity between the ODE systems. \qed
\end{proof}

\begin{keybox}[The Stochastic--Deterministic Bridge]
The stochastic probability distribution $P(s,i,t)$ is constant along the
\emph{deterministic} SIR trajectories. The characteristic curves of the PGF PDE
have the same functional form as the deterministic SIR invariant curves ---
this is an exact identity between the two ODE systems, not a large-$N$ approximation.
\end{keybox}
...
\noindent
\textbf{Remark.} Equation~\eqref{eq:density_decay} gives the correct \emph{exponent}
$d/2$ because the Ward identity renders the theory effectively Gaussian, but the
\emph{amplitude} requires a one-loop calculation and is \emph{universal}
(independent of $\rho_0$): $\rho(t) \sim A_d / (Dt)^{d/2}$, where $A_d$ is a
calculable, $\rho_0$-independent constant. The $\rho_0$ prefactor above is a
schematic placeholder for this universal amplitude $A_d$.
...
\begin{theorem}[Transfer Theorem, Flajolet--Odlyzko \cite{flajoletodlyzko1990}]
...
\emph{connected} diagrams. (We use the statistical mechanics sign convention
$F = -\log Z$; in the QFT convention, $W = \log Z$ generates connected diagrams
directly, matching the AC formula $\hat{C} = \log \hat{A}$.)
This is used in every QFT textbook to discard
...
Consider a \emph{disconnected} vacuum graph $\Gamma$ consisting of two identical,
separate one-loop bubbles in $\phi^4$ theory (not the connected figure-eight
topology). Its total symmetry factor is $|\mathrm{Aut}(\Gamma)| = 8$. This
factorises as $2^2 \times 2$: each one-loop bubble has an internal automorphism
group of order $2^2 = 4$ (from flipping its two propagators), and the factor
of $2 = 2!$ comes from the exchange of the two identical disconnected components.
In EGF language, the exchange factor is exactly the SET construction:
\[
  \hat{C}_{\text{two-bubble}}(z) = \frac{1}{2!}\hat{C}_{\text{one-bubble}}(z)^2
  = \frac{1}{2}\hat{C}_{\text{one-bubble}}(z)^2.
\]
The $1/2!$ is the SET factor for an unordered pair of identical connected
components. More generally, for a disconnected vacuum graph consisting of $k$
copies of the same connected diagram $\gamma$:
\[
...
noted in Cvitanovi\'c, Lautrup \& Pearson \cite{cvitanovic1978} and in Yeats \cite{yeats2017}.
\end{resultbox}
...
In Lagrange language, $\phi(T) = 1/(1-cT)$ has a pole at $T = 1/c$ (the
\emph{Landau pole} of the running coupling), but the Lagrange branch point ---
where the implicit function theorem fails, i.e.\ $1 = z\phi'(T_*)$ --- occurs
at $T_* = 1/(2c)$, giving $z_* = 1/(4c)$. Since $\phi''(T_*) \neq 0$, this is
a \emph{square-root} branch point: $T(z) \sim T_* - \mathrm{const}\cdot(1 - z/z_*)^{1/2}$
near $z_*$. The transfer theorem then gives $[z^n]\,T(z) \sim C \cdot n^{-3/2} \cdot z_*^{-n}$
--- the familiar $(4c)^n n^{-3/2}$ growth. Note that the Lagrange branch point
at $T_* = 1/(2c)$ and the Landau pole at $T = 1/c$ are \emph{different}
singularities; the former governs the large-order asymptotics of the perturbative
coefficients, while the latter signals the breakdown of the resummed coupling.
\end{example}
...
Grossman--Larson \cite{grossmanlarson1989} in the context of numerical analysis (Runge--Kutta
...
This follows from the simplicity of the Weyl algebra $W_1$ (it has no proper
two-sided ideals), which ensures that all faithful representations on polynomial
...
MSET and CYC constructions.

The Connes--Kreimer Hopf algebra $H_R$ is a \emph{different} algebraic structure
from $W_1$: its elements are (forests of) rooted trees, not differential operators.
The connection between the two is that both arise in the perturbative expansion
of the same field theory --- $W_1$ generates the Feynman rules, while $H_R$
organises the renormalisation of the resulting diagrams. This is why the Feynman
diagram expansion of the Doi--Peliti theory and the AC generating function
equations are algebraically the same object at different levels of abstraction.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Kreimer (2000)} \cite{kreimer2000}: \emph{Combinatorics of (perturbative) Quantum
...
\bibitem{cvitanovic1978}
P.\ Cvitanovi\'c, B.\ Lautrup, and R.\ B.\ Pearson,
\newblock Number and weights of {Feynman} diagrams,
\newblock \emph{Phys.\ Rev.\ D}, 18:1939--1949, 1978.

\bibitem{grossmanlarson1989}
R.\ Grossman and R.\ G.\ Larson,
\newblock Hopf-algebraic structure of families of trees,
\newblock \emph{J.\ Algebra}, 126(1):184--210, 1989.

\bibitem{kreimer2000}
D.\ Kreimer,
\newblock Combinatorics of (perturbative) quantum field theory,
\newblock \emph{Phys.\ Rep.}, 363(4--6):387--424, 2002.
\newblock arXiv:hep-th/0010059.
