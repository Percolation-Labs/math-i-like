"""Decoupled deposits experiment: can we parallelize the field?

Tests whether depositing from the layer INPUT (parallelizable via scan)
preserves the field's effectiveness vs depositing from attention OUTPUT
(the proven but serial approach).

Modes:
  baseline        - no field (control)
  tied_output     - tied weights, deposit from attention output (proven best)
  tied_input      - tied weights, deposit from layer input (decoupled)
  tied_input_xlay - tied weights, deposit from input, cross-layer field init

Usage:
    cd sip-sim
    PYTHONUNBUFFERED=1 uv run python experiments/rg/run_decoupled.py
"""
...
# ── Attention with deposit source selection ───────────────────

class FieldAttention(nn.Module):
    """Position-by-position attention with configurable deposit source.

    deposit_from='output': deposit from attention output (faithful, serial)
    deposit_from='input':  deposit from layer input (decoupled, parallelizable)
    """

    def __init__(self, d_model, n_head, dropout, max_len, evap_rate=0.10,
                 layer_idx=0, shared_params=None, deposit_from='output',
                 init_field=None):
        super().__init__()
        self.n_head = n_head
        ...
        self.deposit_from = deposit_from
        ...
        self.terminal_field = None  # for cross-layer passing
    ...
    def forward(self, x, init_field=None):
        B, T, C = x.shape
        ...
        for i in range(T):
            ...
            if self.deposit_from == 'output':
                deposit_src = out_flat
            else:
                deposit_src = x[:, i, :]  # layer input at position i

            deposit = w_dep(deposit_src).view(B, H, d_f)
            field_state = retain * field_state + deposit
            field_states.append(field_state)

        self.terminal_field = field_state.detach()
...
class Block(nn.Module):
    def __init__(self, d_model, n_head, dropout, max_len, evap_rate=0.10,
                 use_field=True, layer_idx=0,
                 shared_params=None, deposit_from='output'):
        super().__init__()
        ...
        if use_field:
            self.attn = FieldAttention(
                ...
                layer_idx=layer_idx, shared_params=shared_params,
    ...
    def forward(self, x, init_field=None):
        x = x + self.attn(self.ln1(x), init_field=init_field)
        x = x + self.mlp(self.ln2(x))
        return x


class GPT_Decoupled(nn.Module):
    def __init__(self, vocab_size, max_len, d_model, n_head, n_layer, dropout,
                 evap_rate=0.10, use_field=True,
                 deposit_from='output', cross_layer_field=False):
        super().__init__()
        self.vocab_size = vocab_size
        self.cross_layer_field = cross_layer_field
        ...
        self.blocks = nn.ModuleList([
            Block(d_model, n_head, dropout, max_len, evap_rate,
                  ...
                  shared_params=shared_params, deposit_from=deposit_from)
    ...
    def forward(self, idx):
        ...
        for block in self.blocks:
            if self.cross_layer_field and prev_field is not None:
                x = block(x, init_field=prev_field)
            else:
                x = block(x)
            if self.cross_layer_field and block.use_field:
                prev_field = block.attn.terminal_field
    ...
    def get_field_info(self):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if hasattr(attn, 'shared_params') and attn.shared_params is not None:
...
def train_and_eval(cfg, vocab_size, n_examples, seed,
...
MODES = {
    "baseline": dict(use_field=False, deposit_from='output',
                     cross_layer_field=False),
    "tied_output": dict(use_field=True, deposit_from='output',
                        cross_layer_field=False),
    "tied_input": dict(use_field=True, deposit_from='input',
                       cross_layer_field=False),
    "tied_input_xlay": dict(use_field=True, deposit_from='input',
                            cross_layer_field=True),
}


def run_experiment():
    print("=" * 70)
    print("DECOUPLED DEPOSITS: Can we break the feedback loop?")
    ...
    for mode_name in ["baseline", "tied_output", "tied_input", "tied_input_xlay"]:
        cfg = MODES[mode_name]
        print(f"\n{'─' * 70}")
        print(f"  {mode_name}")
        print(f"{'─' * 70}")
    ...
    # Summary table
    print(f"\n{'═' * 70}")
    print(f"  SUMMARY: Decoupled Deposits Experiment")
    print(f"{'═' * 70}")
    header = f"  {'Mode':>20s}  {'Params':>7s}"
    for n in n_test_list:
    ...
    for mode_name in ["baseline", "tied_output", "tied_input", "tied_input_xlay"]:
        s = all_results[mode_name]
        line = f"  {mode_name:>20s}  {s['n_params']:>7d}"
        for n in n_test_list:
            r = s['results'][str(n)]
        ...
        if fis and any(f"L0_influence" in fi for fi in fis):
            last = fis[-1]
            vals = [last.get(f"L{l}_influence", 0) for l in range(N_LAYER)]
            mu = np.mean(vals)
            cv = np.std(vals) / (mu + 1e-8) if mu > 0 else 0
            line += f"  {cv:>6.3f}"
        else:
            line += f"  {'---':>6s}"
        print(line)
    ...
    out_path = OUT_DIR / "exp_decoupled_results.json"
